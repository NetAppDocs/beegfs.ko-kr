...
NOTE: 일부 펌웨어 업데이트는 업데이트를 적용하기 위해 재부팅이 필요할 수 있습니다. link:https://networking-docs.nvidia.com/mftswum/4201/mlxfwreset-loading-firmware-on-5th-generation-devices-tool#id-(4.20.1)mlxfwreset–LoadingFirmwareon5thGenerationDevicesTool-mlxfwresetLimitations["NVIDIA의 mlxfwreset 제한 사항"^]을 참조하십시오. 재부팅이 필요한 경우 어댑터를 재설정하는 대신 재부팅을 수행하십시오.
