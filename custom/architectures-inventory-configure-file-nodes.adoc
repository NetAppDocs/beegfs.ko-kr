...
link:https://github.com/NetApp/beegfs/blob/master/getting_started/beegfs_on_netapp/gen2/H2_DAC_IB_A2_EF600/beegfs_cluster_building_blocks/beegfs_ha_building_block_cluster_1/host_vars/beegfs_01.yml["여기"^]을 클릭하여 단일 파일 노드를 나타내는 전체 인벤토리 파일의 예를 확인하십시오.
...
NVIDIA ConnectX-Virtual Protocol Interconnect&reg;(VPI) 어댑터는 InfiniBand와 이더넷을 모두 전송 계층으로 지원합니다. 모드 전환은 자동으로 협상되지 않으므로, `mstconfig`에 포함된 `mstflint` 툴을 사용하여 구성해야 합니다. 이 툴은link:https://network.nvidia.com/products/adapter-software/firmware-tools/["NVIDIA 펌웨어 도구(MFT)"^]의 일부인 오픈 소스 패키지입니다. 어댑터 모드 변경은 한 번만 수행하면 됩니다. 수동으로 변경하거나, 인벤토리의 `eseries-[ib|ib_iser|ipoib|nvme_ib|nvme_roce|roce]_interfaces:` 섹션을 사용하여 구성된 인터페이스의 일부로 Ansible 인벤토리에 포함시켜 자동으로 확인/적용되도록 할 수 있습니다.
