...
----
+
... 지원되는 모든 사항을 기록해 둡니다 link:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/configuring_and_managing_high_availability_clusters/assembly_controlling-cluster-behavior-configuring-and-managing-high-availability-clusters["클러스터 속성"^] 필요한 경우 여기에서 지정할 수도 있습니다. BeeGFS HA 역할이 다양한 테스트를 거친 상태로 배송되므로 이러한 기능을 조정할 필요가 없습니다 link:https://github.com/netappeseries/beegfs/blob/ae038bb2f7617d18a36cc4df8ca9464bd5039b8b/roles/beegfs_ha_7_2/defaults/main.yml#L54["기본값"^].
...
beegfs_ha_fencing_agents:
  fence_redfish:
    ...
    - pcmk_host_list: <HOSTNAME>
      ...
      <<: *redfish
----
... 기타 펜싱 에이전트 구성에 대한 자세한 내용은 를 참조하십시오 link:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/configuring_and_managing_high_availability_clusters/assembly_configuring-fencing-configuring-and-managing-high-availability-clusters["RedHat 문서"^].




. BeeGFS HA 역할은 다양한 튜닝 매개 변수를 적용하여 성능을 더욱 최적화할 수 있습니다. 여기에는 커널 메모리 활용도 최적화 및 블록 디바이스 입출력 등이 포함됩니다. 이 역할은 합당한 일련의 역할을 수행합니다 link:https://github.com/netappeseries/beegfs/blob/release-3.0.1/roles/beegfs_ha_7_2/defaults/main.yml#L106["기본값"^] NetApp E-Series 블록 노드에서 테스트를 기반으로 하지만 기본적으로 다음 사항을 지정하지 않으면 적용되지 않습니다.
+
[source, yaml]
----
...
----
+
.. 필요한 경우 여기에서 기본 성능 튜닝에 대한 변경 사항도 지정합니다. 전체 내용을 참조하십시오 link:https://github.com/netappeseries/beegfs/blob/release-3.0.1/roles/beegfs_ha_7_2/docs/performance_tuning.md["성능 튜닝 매개 변수"^] 설명서를 참조하십시오.
...
. 선택적으로 클러스터의 모든 BeeGFS 서비스에 적용할 구성을 지정합니다. 기본 설정 값을 찾을 수 있습니다 link:https://github.com/netappeseries/beegfs/blob/release-3.0.1/roles/beegfs_ha_7_2/defaults/main.yml#L159["여기"^], 및 서비스별 구성은 다른 곳에서 지정됩니다.
+
.. BeeGFS 관리 서비스:
+
[source, yaml]
...
... 고급 옵션은 에 포함된 기본값 전체 목록을 참조하십시오 link:https://github.com/netappeseries/beegfs/blob/release-3.1.0/roles/beegfs_ha_7_3/defaults/main.yml#L32["BeeGFS HA 역할입니다"^].
...
현재 의 받은 편지함 버전입니다 `opensm` 지원되는 Linux 배포판과 함께 제공되지만 가상화를 지원하지 않습니다. 대신 의 버전을 설치하고 구성해야 합니다 `opensm` Mellanox OpenFabrics Enterprise Distribution(OFED)에서 Ansible을 사용한 구축도 여전히 지원되지만, 몇 가지 추가 단계가 필요합니다.

. curl 또는 원하는 도구를 사용하여 에 나열된 OpenSM 버전의 패키지를 다운로드합니다 link:beegfs-technology-requirements.html["기술 요구 사항"^] 섹션을 Mellanox 웹 사이트에서 으로 이동합니다 `<INVENTORY>/packages/` 디렉토리. 예를 들면 다음과 같습니다.
+
[source, bash]
----
