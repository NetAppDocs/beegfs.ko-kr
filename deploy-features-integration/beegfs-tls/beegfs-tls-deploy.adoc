...
[IMPORTANT]
====
기존 BeeGFS 7 클러스터를 BeeGFS 8로 업그레이드하는 경우 link:../../administer/clusters-upgrade-beegfs-v8.html["BeeGFS 8로 업그레이드"] 절차를 따르십시오. 업그레이드 절차에서 지시가 있을 때까지 클라이언트 TLS를 구성하지 마십시오.
...
----
. CA 인증서를 생성합니다. 이 인증서는 시스템 수명 기간 동안 유효해야 합니다. 인증서가 만료되기 전에 인증서를 재생성할 계획을 세웁니다. 인증서가 만료되면 일부 구성 요소 간의 통신이 실패합니다. TLS 인증서를 업데이트하려면 일반적으로 서비스를 재시작해야 합니다.
+
...
----
+
고유 이름 필드를 CA 구성과 일치하도록 업데이트하고  `IP.1` 및  `IP.2` 값을 관리 서비스 IP 주소로 바꾸십시오.
...
. <<create-management-service-certificates>>에 설명된 것과 동일한 프로세스를 사용하여 클라이언트 인증서를 생성하되, Subject Alternative Name(SAN) 필드에 클라이언트의 IP 주소 또는 호스트 이름을 대체하십시오.
. 클라이언트 인증서를 클라이언트 노드로 복사하고 이름을 `cert.pem`로 변경하십시오:
+
[source, console]
----
...
----
. 다음과 같은 BeeGFS CLI 명령을 실행하여 클라이언트 연결을 확인합니다.
+
[source, console]
----
beegfs health check
