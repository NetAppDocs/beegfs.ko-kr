...
NOTE: 볼륨 크기는 전체 스토리지 풀(볼륨 그룹이라고도 함)의 백분율로 지정됩니다. NetApp은 SSD 오버프로비저닝을 위해 각 풀에 여유 용량을 확보해 두는 것을 강력히 권장합니다(자세한 내용은 https://www.netapp.com/pdf.html?item=/media/17009-tr4800.pdf["NetApp EF600 어레이 소개"^] 참조). 스토리지 풀 `beegfs_m1_m2_m5_m6`은 관리 서비스에 풀 용량의 1%를 할당합니다. 따라서 스토리지 풀 `beegfs_m1_m2_m5_m6`의 메타데이터 볼륨에서 1.92TB 또는 3.84TB 드라이브를 사용하는 경우 이 값을 `21.25`으로 설정하고, 7.65TB 드라이브의 경우 이 값을 `22.25`으로 설정하고, 15.3TB 드라이브의 경우 이 값을 `23.75`으로 설정하십시오.
