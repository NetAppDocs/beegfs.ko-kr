...
. 블록 노드와 파일 노드를 link:beegfs-technology-requirements.html#file-node-requirements["1m InfiniBand HDR 200GB 직접 연결 구리 케이블"]을 사용하여 연결하여 다음 그림에 표시된 토폴로지와 일치하도록 합니다.
+
...
. 파일 노드의 나머지 InfiniBand 포트를 InfiniBand 스토리지 스위치에 맞는 link:beegfs-technology-requirements.html#storage-network-switch-requirements["2M InfiniBand 케이블"]을 사용하여 스토리지 네트워크의 InfiniBand 스위치에 연결하십시오.
+
